...
(2, 'Backend Developer', 'Python, MySQL, Docker'),
(3, 'QA Engineer', 'Testing, SQL, Postman');
```

После этого была выполнена проверка добавления записи с несуществующим пользователем:
...
При выполнении запроса MySQL выдал ошибку 1452, так как пользователя с id = 999 не существует. Это подтверждает, что внешний ключ userid не позволяет добавить резюме для несуществующего пользователя.

## Задание 10

Сначала был удален пользователь с id `3`.

```sql
DELETE FROM simpledb.users
WHERE id = 3;
```

Резюме пользователя №3 тоже исчезло автоматически благодаря внешнему ключу с параметром ON DELETE CASCADE.

После этого был изменён id пользователя с `2` на `20`.
...
После этого `userid` в связанной записи таблицы `resume` тоже изменился на `20`.

![Изменение id](зад-10.png)
