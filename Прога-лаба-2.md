...
def gen_bin_tree(height: int = 4, root: int = 12) -> dict[int, list[Any]]:
    """Строит бинарное дерево в виде словаря.

    Левый потомок: root ** 3, правый потомок: root * 2 - 1.
    ...
    if height == 0:
        return {root: []}
    left = gen_bin_tree(height - 1, root ** 3)
...
def gen_bin_tree(height: int = 4, root: int = 12) -> Optional[Node]:
